...
public class CallLog {
	...
	public CallLog(String name, Date callDate, PhoneNumber number) {
		//Date d = new Date(callDate.getTime()); 		WHYYY
		this.name = name;
		this.log = new ArrayList<>(1);
		this.log.add(new CallLogEntry(callDate, number));
	...
	public List<CallLogEntry> callsAfter(Date after) {
		...
		for (CallLogEntry e : this.log)
		{
			...
			{
				CallLogEntry c = new CallLogEntry (new Date(d.getTime()), e.getNumber());
				result.add(c);
			}
		}
